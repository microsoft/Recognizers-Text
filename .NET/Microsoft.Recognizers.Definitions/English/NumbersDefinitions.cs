...
namespace Microsoft.Recognizers.Definitions.English
{
	...
	public static class NumbersDefinitions
	{
		...
		public static readonly Func<string, string> NumbersWithPlaceHolder = (placeholder) => $@"((minus|negative)\s+)?(((?<!\d+\s*)-\s*)|(?<=\b))\d+(?!(\.\d+[a-zA-Z]))(?={placeholder})";
		public static readonly string NumbersWithSuffix = $@"({NegativeNumberTermsRegex})?(((?<!\d+\s*)-\s*)|(?<=\b))\d+\s*(K|k|M|T|G)(?=\b)";
		public static readonly string RoundNumberIntegerRegexWithLocks = $@"({NegativeNumberTermsRegex})?(?<=\b)\d+\s+{RoundNumberIntegerRegex}(?=\b)";
		public static readonly string NumbersWithDozenSuffix = $@"({NegativeNumberTermsRegex})?(((?<!\d+\s*)-\s*)|(?<=\b))\d+\s+dozen(s)?(?=\b)";
		public static readonly string AllIntRegexWithLocks = $@"((?<=\b)(({NegativeNumberTermsRegex})?{AllIntRegex})(?=\b))";
		public static readonly string AllIntRegexWithDozenSuffixLocks = $@"(?<=\b)({NegativeNumberTermsRegex})?(((half\s+)?a\s+dozen)|({AllIntRegex}\s+dozen(s)?))(?=\b)";
		...
		public static readonly string OrdinalSuffixRegex = $@"(?<=\b)({NegativeNumberTermsRegex})?((\d*(1st|2nd|3rd|4th|5th|6th|7th|8th|9th|0th))|(11th|12th))(?=\b)";
		public static readonly string OrdinalNumericRegex = $@"(?<=\b)({NegativeNumberTermsRegex})?(\d{{1,3}}(\s*,\s*\d{{3}})*\s*th)(?=\b)";
		public static readonly string OrdinalRoundNumberRegex = $@"(?<!(a|an)\s+){RoundNumberOrdinalRegex}";
		public static readonly string OrdinalEnglishRegex = $@"(?<=\b)({NegativeNumberTermsRegex})?{AllOrdinalRegex}(?=\b)";
		public static readonly string FractionNotationWithSpacesRegex = $@"(({NegativeNumberTermsRegex})?(((?<=\W|^)-\s*)|(?<=\b))\d+\s+\d+[/]\d+(?=(\b[^/]|$)))";
		public static readonly string FractionNotationRegex = $@"({NegativeNumberTermsRegex})?(((?<=\W|^)-\s*)|(?<=\b))\d+[/]\d+(?=(\b[^/]|$))";
		public static readonly string FractionNounRegex = $@"(?<=\b)({NegativeNumberTermsRegex})?({AllIntRegex}\s+(and\s+)?)?({AllIntRegex})(\s+|\s*-\s*)((({AllOrdinalRegex})|({RoundNumberOrdinalRegex}))s|halves|quarters)(?=\b)";
		public static readonly string FractionNounWithArticleRegex = $@"(?<=\b)({NegativeNumberTermsRegex})?({AllIntRegex}\s+(and\s+)?)?(a|an|one)(\s+|\s*-\s*)(?!\bfirst\b|\bsecond\b)(({AllOrdinalRegex})|({RoundNumberOrdinalRegex})|half|quarter)(?=\b)";
		public static readonly string FractionPrepositionRegex = $@"(?<=\b)({NegativeNumberTermsRegex})?(({AllIntRegex})|((?<!\.)\d+))\s+over\s+(({AllIntRegex})|(\d+)(?!\.))(?=\b)";
		public static readonly string AllPointRegex = $@"((\s+{ZeroToNineIntegerRegex})+|(\s+{SeparaIntRegex}))";
		public static readonly string AllFloatRegex = $@"{AllIntRegex}(\s+point){AllPointRegex}";
		public static readonly string DoubleWithMultiplierRegex = $@"({NegativeNumberTermsRegex})?(((?<!\d+\s*)-\s*)|((?<=\b)(?<!\d+\.)))\d+\.\d+\s*(K|k|M|G|T|B|b)(?=\b)";
		public static readonly string DoubleExponentialNotationRegex = $@"({NegativeNumberTermsRegex})?(((?<!\d+\s*)-\s*)|((?<=\b)(?<!\d+\.)))(\d+(\.\d+)?)e([+-]*[1-9]\d*)(?=\b)";
		public static readonly string DoubleCaretExponentialNotationRegex = $@"({NegativeNumberTermsRegex})?(((?<!\d+\s*)-\s*)|((?<=\b)(?<!\d+\.)))(\d+(\.\d+)?)\^([+-]*[1-9]\d*)(?=\b)";
		...
		public static readonly string DoubleWithRoundNumber = $@"({NegativeNumberTermsRegex})?(((?<!\d+\s*)-\s*)|((?<=\b)(?<!\d+\.)))\d+\.\d+\s+{RoundNumberIntegerRegex}(?=\b)";
		public static readonly string DoubleAllFloatRegex = $@"((?<=\b)({NegativeNumberTermsRegex})?{AllFloatRegex}(?=\b))";
