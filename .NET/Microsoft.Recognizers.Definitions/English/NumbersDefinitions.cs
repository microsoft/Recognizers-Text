...
namespace Microsoft.Recognizers.Definitions.English
{
	...
	public static class NumbersDefinitions
	{
		...
		public const string MoreRegex = @"(bigger|greater|more|higher|larger|>)";
		public const string LessRegex = @"(less|lower|smaller|<)";
		public const string EqualRegex = @"(equal(\s+to)?|=)";
		...
		public static readonly string OneNumberRangeMoreRegex1 = $@"({MoreOrEqual}|{MoreRegex})\s*(than\s*)?(?<number1>((?!((\.(?!\d+))|(,(?!\d+))|and)).)+)";
		public const string OneNumberRangeMoreRegex2 = @"(?<number1>((?!((\.(?!\d+))|(,(?!\d+)))).)+)\s*and\s+(more|greater|higher|larger|bigger)";
		public static readonly string OneNumberRangeLessRegex1 = $@"({LessOrEqual}|{LessRegex})\s*(than\s*)?(?<number2>((?!((\.(?!\d+))|(,(?!\d+))|and)).)+)";
		public const string OneNumberRangeLessRegex2 = @"(?<number2>((?!((\.(?!\d+))|(,(?!\d+)))).)+)\s*and\s+(less|lower|smaller)";
