...
namespace Microsoft.Recognizers.Definitions
{
	...
	public static class BasePhoneNumbers
	{
		...
		public const string USPhoneNumberRegex = @"((((\B\+)|\b)1(\s|-)?)|\b)?(\(\d{3}\)\s?|\b\d{3}\s?[-\.]?\s?)\d{3}\s?[-\.]?\s?\d{4}(x\d{3,5})?(?!-)\b";
		...
		public const string NLPhoneNumberRegex = @"((((\(\s?(\+\s?|00)31\s?\)\s?)|(((?<!\d)\+\s?|\b00)31\s?))?(((\b|(?<=31))0?\d{1,3}\s?-?|\(\s?0?\d{1,3}\s?\)|\(0\)\s?\d{1,3}\s?-?)\s?[\d\s]{5,11}\d))|\b\d{10,12})(?!-)\b";
