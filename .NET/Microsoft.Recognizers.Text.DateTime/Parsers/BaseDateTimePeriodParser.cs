...
namespace Microsoft.Recognizers.Text.DateTime
{
    public class BaseDateTimePeriodParser : IDateTimeParser
    {
        ...
        private DateTimeResolutionResult MergeDateWithSingleTimePeriod(string text, DateObject referenceTime)
        {
            ...
            {
                var parserConfig = this.Config as English.EnglishTimePeriodParserConfiguration;
                if (parserConfig != null)
                {
                    if (parserConfig.TimePeriodFromFor.Match(ers[0].Text).Success)
                    {
                        var durationERs = this.Config.DurationExtractor.Extract(ers[0].Text);

                        if (durationERs.Count > 0)
                        {
                            return MergeDurationWithSingleTimePoint(ers[0].Text, referenceTime);
                        }
                    }
                }
        ...
        private DateTimeResolutionResult MergeDurationWithSingleTimePoint(string text, DateObject referenceTime)
        {
            var ret = new DateTimeResolutionResult();
