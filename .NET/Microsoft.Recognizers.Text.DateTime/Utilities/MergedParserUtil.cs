...
namespace Microsoft.Recognizers.Text.DateTime
{
    public static class MergedParserUtil
    {
        ...
        public static DateTimeParseResult TasksModeModification(DateTimeParseResult slot, DateObject referenceTime)
        {
            switch (slot.Type.Substring(ParserTypeName.Length + 1))
            {
                ...
                case Constants.SYS_DATETIME_TIMEPERIOD:
                    slot = HandlePastTimePeriod(slot, referenceTime);
                    break;

                case Constants.SYS_DATETIME_DATETIME:
                    slot = HandlePastDateTime(slot, referenceTime);
                    break;

                case Constants.SYS_DATETIME_DATETIMEPERIOD:
                    slot = HandlePastDateTimePeriod(slot, referenceTime);
                    break;
            }
        ...
        private static DateTimeParseResult HandlePastDateTime(DateTimeParseResult slot, DateObject referenceTime)
        {
            if (!slot.TimexStr.Contains("XXXX"))
            {
                return slot;
        ...
        private static DateTimeParseResult HandlePastTime(DateTimeParseResult slot, DateObject referenceTime)
        {
            var value = (SortedDictionary<string, object>)slot.Value;
        ...
        private static DateTimeParseResult HandlePastTimePeriod(DateTimeParseResult slot, DateObject referenceTime)
        {
            var value = (SortedDictionary<string, object>)slot.Value;
