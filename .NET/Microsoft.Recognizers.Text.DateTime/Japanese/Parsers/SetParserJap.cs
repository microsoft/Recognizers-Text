...
{
    public class SetParserJap : IDateTimeParser
    {
        ...
        private static readonly IDateTimeExtractor TimeExtractor = new TimeExtractorJap();
        private static readonly IDateTimeExtractor DateExtractor = new DateExtractorJap();
        private static readonly IDateTimeExtractor DateTimeExtractor = new DateTimeExtractorJap();

        private readonly IFullDateTimeParserConfiguration config;

        public SetParserJap(IFullDateTimeParserConfiguration configuration)
        {
        ...
        private DateTimeResolutionResult ParseEachDuration(string text, DateObject refDate)
        {
            ...
            if (SetExtractorJap.EachPrefixRegex.IsMatch(beforeStr))
            {
        ...
        private DateTimeResolutionResult ParseEachUnit(string text)
        {
            var ret = new DateTimeResolutionResult();
            // handle "each month"
            var match = SetExtractorJap.EachUnitRegex.Match(text);
        ...
        private DateTimeResolutionResult ParserTimeEveryday(string text, DateObject refDate)
        {
            ...
            var beforeStr = text.Substring(0, ers[0].Start ?? 0);
            var match = SetExtractorJap.EachDayRegex.Match(beforeStr);
            if (match.Success)
            {
        ...
        private DateTimeResolutionResult ParseEachDate(string text, DateObject refDate)
        {
            ...
            var beforeStr = text.Substring(0, ers[0].Start ?? 0);
            var match = SetExtractorJap.EachPrefixRegex.Match(beforeStr);
            if (match.Success)
            {
