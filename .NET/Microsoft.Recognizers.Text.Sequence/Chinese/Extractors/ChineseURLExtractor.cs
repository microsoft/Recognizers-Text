...
{
    public class ChineseURLExtractor : BaseSequenceExtractor
    {
        public ChineseURLExtractor()
        {
            var regexes = new Dictionary<Regex, string>
            {
                {
                    new Regex(URLDefinitions.IpUrlRegex, RegexOptions.Compiled),
                    Constants.URL_REGEX
                },
                {
                    new Regex(URLDefinitions.UrlRegex, RegexOptions.Compiled),
                    Constants.URL_REGEX
                },
                {
                    new Regex(URLDefinitions.UrlRegex2, RegexOptions.Compiled),
                    Constants.URL_REGEX
                },
            };

            Regexes = regexes.ToImmutableDictionary();
            AmbiguousTimeTerm = new Regex(URLDefinitions.AmbiguousTimeTerm, RegexOptions.Compiled);

            TldMatcher = new StringMatcher();
            TldMatcher.Init(URLDefinitions.TldList);
        }
