    ...
    public class BotBuiderV4Bot : IBot
    {
        ...
        public async Task OnTurnAsync(ITurnContext turnContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message)
            {
                ...
                //await UpdateDeliveryState(luisResults, dc.Context);
                ...
                if (!dc.Context.Responded)
                {
                    ...
                    switch (dialogResult.Status)
                    {
                        case DialogTurnStatus.Empty:
                            ...
                            //{
                            //    case GreetingIntent:
                            await dc.BeginDialogAsync(nameof(DeliveryDialog));
                            break;
        ...
        /// </summary>
        private async Task UpdateDeliveryState(RecognizerResult luisResult, ITurnContext turnContext)
        {
            if (luisResult.Entities != null && luisResult.Entities.HasValues)
            {
                // Get latest GreetingState
                var deliveryState = await _deliveryStateAccessor.GetAsync(turnContext, () => new DeliveryState());
                ...
                foreach (var name in userNameEntities)
                {
                    ...
                    if (entities[name] != null)
                    {
                        ...
                        deliveryState.Quantity = char.ToUpper(newName[0]) + newName.Substring(1);
                        break;
                    }
                }
                ...
                foreach (var city in userLocationEntities)
                {
                    if (entities[city] != null)
                    {
                        ...
                        deliveryState.Date = char.ToUpper(newCity[0]) + newCity.Substring(1);
                        break;
                    }
                }

                // Set the new values into state.
                await _deliveryStateAccessor.SetAsync(turnContext, deliveryState);
            }
        }
    }
}
