...
namespace Microsoft.Recognizers.Definitions
{
    ...
    public static class BasePhoneNumbers
    {
      ...
      public static readonly Func<string, string, string> EUPhoneNumberRegex = (WordBoundariesRegex, EndWordBoundariesRegex) => $@"((((\(\s?(\+\s?|00)(31|46)\s?\)\s?)|(((?<!\d)\+\s?|{WordBoundariesRegex}00)(31|46)\s?))?((({WordBoundariesRegex}|(?<=(31|46)))0?\d{{1,3}}|\(\s?0?\d{{1,3}}\s?\)|\(0\)[-\s]?\d{{1,3}})\s?-?[\d\s]{{5,11}}\d))|\b\d{{10,12}})(?!-){EndWordBoundariesRegex}";
