...
namespace Microsoft.Recognizers.Definitions
{
    ...
    public static class BasePhoneNumbers
    {
      ...
      public static readonly Func<string, string, string> SEPhoneNumberRegex = (WordBoundariesRegex, EndWordBoundariesRegex) => $@"(((\+|00)46\s(8|\d{{2,3}})\s\d{{2,3}}\s\d{{2,3}}(\s\d{{2}})?{EndWordBoundariesRegex}|(08|0\d{{2,3}})-\d{{2,3}}\s\d{{2,3}}(\s\d{{2}})?{EndWordBoundariesRegex}))";
