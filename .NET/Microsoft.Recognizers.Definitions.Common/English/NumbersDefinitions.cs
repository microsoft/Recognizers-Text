...
namespace Microsoft.Recognizers.Definitions.English
{
    ...
    public static class NumbersDefinitions
    {
      ...
      public static readonly string OneNumberRangeMoreRegex1 = $@"(?<!no\s+)({MoreOrEqual}|{MoreRegex})\s*(the\s+)?(?<number1>({NumberSplitMark}.)+)";
      ...
      public static readonly string OneNumberRangeLessRegex1 = $@"(?<!no\s+)({LessOrEqual}|{LessRegex})\s*(the\s+)?(?<number2>({NumberSplitMark}.)+)";
