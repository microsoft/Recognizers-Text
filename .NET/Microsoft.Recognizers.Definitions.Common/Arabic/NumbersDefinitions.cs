...
namespace Microsoft.Recognizers.Definitions.Arabic
{
    ...
    public static class NumbersDefinitions
    {
      ...
      public const string RoundNumberIntegerRegex = @"(ال)?(?:مائتان|مائة|مائة|مائتين|ثلاثمائه|أربعة مئة|خمسمائة|ستمائة|سبعمائة|ثمان مائة|تسعمائة|تريليون|ترليون|آلاف|تريليونين|تريليونات|مليار|ملياري|مليارات|مليون|مليونان|ملايين|ملايين|ألف|مليونين|ألفين|مئة|ومائتين|الفين|الآف)";
      public const string ZeroToNineIntegerRegex = @"(بإثنان|وخمسة|وواحد|واحد|وأربعة|واثنان|اثنان|وثلاثة|ثلاثة|واربعة|أربعة|خمسة|وستة|ستة|وسبعة|سبعة|وثمانية|ثمانية|وتسعة|تسع|أحد|اثني|ثلاث|صفر|سبع|ست|اربع|السادس|الثامنة|تسعة|اثنين|واحدُ|وواحدُ:?)";
      ...
      public const string TenToNineteenIntegerRegex = @"(ال)?(?:((ثلاث|ثلاثة|سبعة|ثمان|ثمانية|أربع|أربعة|خمسة|تسعة|اثنان|اثنان|اثنين|اثتنين|اثنتان|ست|ستة|أحد|أربعة|اثني)\s(عشر|عشرة)))";
      public const string TensNumberIntegerRegex = @"(عشرة|عشرون|ثلاثون|أربعون|خمسون|ستون|سبعون|ثمانون|تسعين|عشر|وعشرين|وثلاثين|وأربعين|وخمسين|وستين|وسبعين|وثمانين|وتسعين|وعشرون|وثلاثون|وأربعون|وخمسون|وستون|وسبعون|وثمانون|وتسعون|عشرين|ثلاثين|أربعين|خمسين|ستين|سبعين|ثمانين|تسعون:?)";
      public static readonly string SeparaIntRegex = $@"(?:(({TenToNineteenIntegerRegex}|({ZeroToNineIntegerRegex}(\s+(و)?|\s*-\s*){TensNumberIntegerRegex})|{TensNumberIntegerRegex}|{ZeroToNineIntegerRegex}|{RoundNumberIntegerRegex})(\s+{RoundNumberIntegerRegex})*))|(((\s+{RoundNumberIntegerRegex})+))";
      ...
      public static readonly Func<string, string> NumbersWithPlaceHolder = (placeholder) => $@"(((?<!{{?[\u0660-\u0669]}}+\s*)-\s*)|(?<=\b))?[\u0660-\u0669]+(?!([\،]?[\u0660-\u0669]+[\u0621-\u064A]))(?={placeholder})";
      public static readonly string NumbersWithSuffix = $@"((((?<![\u0660-\u0669]+\s*)-\s*)|(?<=\b))?[\u0660-\u0669])+\s*{BaseNumbers.NumberMultiplierRegex}(?=\b)";
      public static readonly string RoundNumberIntegerRegexWithLocks = $@"((?<!{{?[\u0660-\u0669]}}))?[\u0660-\u0669]+\s+{RoundNumberIntegerRegex}(?=\b)";
      public const string NumbersWithDozenSuffix = @"(((((?<!{?[\u0660-\u0669]}+\s*)-\s*)|(?<=\b))?[\u0660-\u0669]+(?!([\،]?[\u0660-\u0669])))+\s+(دستة|دستات|دست|دزينة|دزينات))|(دزينتين)(?=\b)";
      public static readonly string AllIntRegexWithLocks = $@"((?<=\b){AllIntRegex}(?=\b))";
      public static readonly string AllIntRegexWithDozenSuffixLocks = $@"(?<=\b)(((نصف\s+)?(دزينتين|دزينات|دستة|دزينة))|(({AllIntRegex}\s)(و?)(دزينة|دستة|دستات|دست|دزينات|دزينتين)))(?=\b)";
      public const string RoundNumberOrdinalRegex = @"(?:((من|على)\s+))";
      public const string NumberOrdinalRegex = @"(المئتيان|مائتي|الحاديه عشر|سابعًا|خامسا|ثانيا|أول|الأول|الثاني|الثالث|الرابع|الخامس|السادس|السابع|الثامن|التاسع|الأولى|الثانية|الثالثة|الرابعة|الخامسة|السادسة|السابعة|الثامنة|التاسعة|السادس عشر|السابعة عشرة|السادسة عشرة|الثالثة عشرة|الحادية عشرة|السابع عشر|سادس عشر|الخامس عشر|الحادية عَشْرةَ|الثانيَ عَشَر|الثانيةَ عَشْرةَ|الثالثَ عَشَرَ|الثالثةَ عَشْرةَ|الرابعَ عَشَرَ|الرابعةَ عَشْرةَ|الخامِسَ عَشَرَ|الخامسةَ عَشْرةَ|السادِسَ عَشَرَ|السادسةَ عَشْرةَ|السابعَ عَشَرَ|السابعةَ عَشْرةَ|الثامنَ عَشَرَ|الثامنةَ عَشْرةَ|التاسعَ عَشَرَ|التاسعةَ عَشْرةَ|الحادِيَ عَشَرَ|الحادي عشر|الثاني عشر|الثالث عشر|الرابع عشر|الثامن عشر|التاسع عشر|الثانية عشرة|الرابعة عشرة|الخامسة عشرة|الثامنة عشرة|التاسعة عشرة|العاشر|العاشرة|عشرون|العشرين|الثلاثين|الثلاثون|الرابعة والأربعون|الرابع والأربعون|خمسون|الخمسون|الستين|ستون|والستين|سبعون|السبعون|والسبعون|ثامن عشر|الثامن عشر|الرابع والأربعين|الثامنة والثمانون|الثامن|والثمانين|تسعون| التاسع والتسعون|التاسعة والتسعون:?)";
      ...
      public static readonly string AllFloatRegex = $@"({AllIntRegex}\s+(نقطة|جزء)\s+({AllPointRegex}|{{AllOrdinalRegex}}))";
      public static readonly string DoubleWithMultiplierRegex = $@"(((?<![\u0660-\u0669]+\s*)-\s*)|((?<=\b)(?<![\u0660-\u0669]+[\,])))?[\u0660-\u0669]+[\,][\u0660-\u0669]+\s*{BaseNumbers.NumberMultiplierRegex}(?=\b)";
      public const string DoubleExponentialNotationRegex = @"([\u0660-\u0669]+([\,]?[\u0660-\u0669]+)?)e([\u0661-\u0669]?[\u0660-\u0669]*[+-]*)";
      public const string DoubleCaretExponentialNotationRegex = @"([\u0660-\u0669]+([\,]?[\u0660-\u0669]+)?)\^([\u0661-\u0669]?[\u0660-\u0669]*[+-]*)";
      public static readonly Func<string, string> DoubleDecimalPointRegex = (placeholder) => $@"(((?<![\u0660-\u0669]+\s*)-\s*)|((?<=\b)(?<![\u0660-\u0669]+[\,])))?[\u0660-\u0669]+[\,][\u0660-\u0669]+(?!([\,]?[\u0660-\u0669]+))(?={placeholder})";
      public static readonly Func<string, string> DoubleWithoutIntegralRegex = (placeholder) => $@"(?<=\s|^)(?<!([\u0660-\u0669]+))[\,][\u0660-\u0669]+(?!([\,][\u0660-\u0669]+))(?={placeholder})";
      public static readonly string DoubleWithRoundNumber = $@"(((?<![\u0660-\u0669]+\s*)-\s*)|((?<=\b)(?<![\u0660-\u0669]+[\,])))?[\u0660-\u0669]+[\,][\u0660-\u0669]+\s+{RoundNumberIntegerRegex}(?=\b)";
