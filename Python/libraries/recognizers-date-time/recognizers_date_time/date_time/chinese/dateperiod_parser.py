...
class ChineseDatePeriodParser(BaseDatePeriodParser):
    def __init__(self):
        ...
        self.year_to_year_dependence_regex = RegExpUtility.get_safe_reg_exp(ChineseDateTime.YearToYearDependence)
    ...
    def _parse_year_to_year(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        if not match:
            match = regex.search(self.year_to_year_dependence_regex, source)
            if not match:
                return result
