...
class JapaneseNumberParserConfiguration(NumberParserConfiguration):
    @property
    ...
    def __init__(self, culture_info=None):
        ...
        self.zero_to_nine_map_jpn = JapaneseNumeric.ZeroToNineMap
        self.round_number_map_jpn = JapaneseNumeric.RoundNumberMapChar
        self.full_to_half_map_jpn = JapaneseNumeric.FullToHalfMap
        self.trato_sim_map_jpn = None
        self.unit_map_jpn = JapaneseNumeric.UnitMap
        self.round_direct_list_jpn = JapaneseNumeric.RoundDirectList
        self.digit_num_regex = JapaneseNumeric.DigitNumRegex
        self.dozen_regex = JapaneseNumeric.DozenRegex
        self.percentage_regex = JapaneseNumeric.PercentageRegex
        self.double_and_round_jpn_regex = RegExpUtility.get_safe_reg_exp(JapaneseNumeric.DoubleAndRoundRegex)
        self.frac_split_regex = RegExpUtility.get_safe_reg_exp(JapaneseNumeric.FracSplitRegex)
        self._negative_number_sign_regex = RegExpUtility.get_safe_reg_exp(JapaneseNumeric.NegativeNumberSignRegex)
        self.point_regex_jpn = JapaneseNumeric.PointRegex
        self.spe_get_number_regex = RegExpUtility.get_safe_reg_exp(JapaneseNumeric.SpeGetNumberRegex)
        self.pair_regex = RegExpUtility.get_safe_reg_exp(JapaneseNumeric.PairRegex)
        self.round_number_integer_regex = RegExpUtility.get_safe_reg_exp(JapaneseNumeric.RoundNumberIntegerRegex)
...
class JapaneseNumberParser(BaseNumberParser):
    def __init__(self, config: JapaneseNumberParserConfiguration):
    ...
    def parse(self, source: ExtractResult) -> Optional[ParseResult]:
        ...
        if 'Per' in extra:
            result = self.per_parse_jpn(simplified_source)
        elif 'Num' in extra:
            ...
            result = self.frac_parse_jpn(simplified_source)
        elif 'Dou' in extra:
            result = self.dou_parse_jpn(simplified_source)
        elif 'Integer' in extra:
            result = self.int_parse_jpn(simplified_source)
        elif 'Ordinal' in extra:
            result = self.ord_parse_jpn(simplified_source)
    ...
    def replace_trad_with_simplified(self, source: str) -> str:
        if source is None or not source.strip():
            return source
        if self.config.trato_sim_map_jpn is None:
            return source
        return ''.join(map(lambda c: self.config.trato_sim_map_jpn.get(c, c), source))

    def replace_full_with_half(self, source: str) -> str:
        if source is None or not source.strip():
            return source
        return ''.join(map(lambda c: self.config.full_to_half_map_jpn.get(c, c), source))

    def replace_unit(self, source: str) -> str:
        if source is None or not source.strip():
            return source
        for (k, v) in self.config.unit_map_jpn.items():
            source = source.replace(k, v)
        return source

    def per_parse_jpn(self, source: ExtractResult) -> ParseResult:
        ...
        if 'Spe' in source.data:
            ...
            if source_text == '半額' or source_text == '半折':
                ...
                if len(matches) == 2:
                    ...
                    if int_number_char == '対' or int_number_char == '对':
                        ...
                        int_number = self.config.zero_to_nine_map_jpn[int_number_char]
                    ...
                    if point_number_char == '半':
                        ...
                        point_number = self.config.zero_to_nine_map_jpn[point_number_char] * 0.1
                    ...
                    point_number = self.config.zero_to_nine_map_jpn[point_number_char] * 0.1
                    dot_number = self.config.zero_to_nine_map_jpn[dot_number_char] * 0.01
            ...
            if any(x for x in ['k', 'K', 'ｋ', 'Ｋ'] if x in double_text):
                ...
                power = 1000000000000
            result.value = self.get_digit_value_jpn(double_text, power)

        else:
            double_match = regex.search(self.config.percentage_regex, source_text)
            double_text = self.replace_unit(double_match.group())

            split_result = regex.split(self.config.point_regex_jpn, double_text)
            if split_result[0] == '':
                split_result[0] = '零'

            double_value = self.get_int_value_jpn(split_result[0])
            if len(split_result) == 2:
                if regex.search(self.config.negative_number_sign_regex, split_result[0]) is not None:
                    double_value -= self.get_point_value_jpn(split_result[1])
                else:
                    double_value += self.get_point_value_jpn(split_result[1])
    ...
    def frac_parse_jpn(self, source: ExtractResult) -> ParseResult:
        result = ParseResult(source)
    ...
    def get_value_from_part(self, part: str) -> float:
        if self.is_digit_jpn(part):
            return self.get_digit_value_jpn(part, 1.0)
        return self.get_int_value_jpn(part)

    def dou_parse_jpn(self, source: ExtractResult) -> ParseResult:
        result = ParseResult(source)

        source_text = self.replace_unit(source.text)

        if (regex.search(self.config.double_and_round_jpn_regex, source.text)) is not None:
            power = self.config.round_number_map_jpn[source_text[-1:]]
            result.value = self.get_digit_value_jpn(source_text[:-1], power)
        else:
            split_result = regex.split(self.config.point_regex_jpn, source_text)
            if split_result[0] == '':
                split_result[0] = '零'
            if regex.search(self.config.negative_number_sign_regex, split_result[0]) is not None:
                result.value = self.get_int_value_jpn(split_result[0]) - self.get_point_value_jpn(split_result[1])
            else:
                result.value = self.get_int_value_jpn(split_result[0]) + self.get_point_value_jpn(split_result[1])
    ...
    def int_parse_jpn(self, source: ExtractResult) -> ParseResult:
        result = ParseResult(source)
        result.value = self.get_int_value_jpn(source.text)
        result.resolution_str = self.__format(result.value)
        return result

    def ord_parse_jpn(self, source: ExtractResult) -> ParseResult:
        result = ParseResult(source)
        source_text = source.text[1:]

        if regex.search(self.config.digit_num_regex, source_text) is not None:
            result.value = self.get_digit_value_jpn(source_text, 1)
        else:
            result.value = self.get_int_value_jpn(source_text)

        result.resolution_str = self.__format(result.value)
        return result

    def get_digit_value_jpn(self, source: str, power: float) -> float:
        negative: bool = False
        result_str = source
    ...
    def get_int_value_jpn(self, source: str) -> float:
        result_str = source
        dozen = False
        pair = False
        ...
        if regex.search(self.config.dozen_regex, result_str) is not None:
            ...
            #   result_str = result_str[:-1]
            if isinstance(self.config, JapaneseNumberParserConfiguration):
        ...
        for i in range(len(result_str)):
            c = result_str[i]
            if c in self.config.round_number_map_jpn:
                round_recent = self.config.round_number_map_jpn[c]
                if not has_number:
                    before_value = 1
                if round_before != -1 and round_recent > round_before:
                    ...
                    if i == len(result_str)-1 or c in self.config.round_direct_list_jpn:
            ...
            elif c in self.config.zero_to_nine_map_jpn:
                has_number = True
                if i != len(result_str)-1:
                    if c == '零' and result_str[i+1] not in self.config.round_number_map_jpn:
                        round_default = 1
                    else:
                        before_value = before_value * 10 + self.config.zero_to_nine_map_jpn[c]
                        is_round_before = False
                else:
                    part_value += before_value * 10
                    part_value += self.config.zero_to_nine_map_jpn[c] * round_default
                    int_value += part_value
                    part_value = 0
        if negative:
            int_value = - int_value
    ...
    def get_point_value_jpn(self, source: str) -> float:
        result: float = 0
        scale: float = 0.1
        for c in source:
            result += scale * self.config.zero_to_nine_map_jpn[c]
            scale *= 0.1
        return result

    def is_digit_jpn(self, source: str) -> bool:
