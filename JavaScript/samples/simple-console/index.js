...
function runRecognition() {
    ...
    consoleWriteLn('Enter the text to recognize: ');
    ...
    stdin.addListener('data', function (e) {
        ...
        if (input && input.length > 0) {
            ...
            if (input.toLowerCase() === 'exit') {
                ...
                consoleWriteLn();
                consoleWriteLn(results.length > 0 ? "I found the following entities (" + results.length + ")" : "I found no entities");
                consoleWriteLn();
                [].concat.apply([], results).forEach(function (result) {
                    consoleWriteLn(JSON.stringify(result, null, "\t"));
                });
                consoleWriteLn();
            }
...
// Show Introduction
function showIntro() {
    consoleWriteLn("Welcome to the Recognizer's Sample console application!");
    consoleWriteLn("To try the recognizers enter a phrase and let us show you the different outputs for each recognizer or just type 'exit' to leave the application.");
    consoleWriteLn();
    consoleWriteLn("Here are some examples you could try:");
    consoleWriteLn();
    consoleWriteLn("\" I want twenty meters of cable for tomorrow \"");
    consoleWriteLn("\" I'll be available tomorrow from 11am to 2pm to receive up to 5kg of sugar\"");
    consoleWriteLn("\" I'll be out between 4 and 22 this month \"");
    consoleWriteLn("\" I was the fifth person to finish the 10 km race \"");
    consoleWriteLn("\" The temperature this night will be of 40 deg celsius \"");
    consoleWriteLn("\" The american stock exchange said a seat was sold for down $ 5,000 from the previous sale last friday \"");
    consoleWriteLn("\" It happened when the baby was only ten months old \"");
    consoleWriteLn();
}
