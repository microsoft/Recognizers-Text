...
export namespace FrenchNumeric {
    ...
    export const NumberWithSuffixPercentage = `(?<!%)(${BaseNumbers.NumberReplaceToken})(\\s*)(%(?!${BaseNumbers.NumberReplaceToken})|(pourcent(s|tages?)?)\\b)`;
    ...
    export const TillRegex = `((?<!\\b[èe]ga(l|ux)\\s+)[àa]|--|-|—|——|~|–)`;
    export const MoreRegex = `(?:(bigger|greater|more|plus(\\s+(haut|grand|âgée?))?|sup[ée]rieur)(\\s+([àa]|que))?|dépassant|(au-dessus|(a\\s+)?plus|dépassant|au-delà)\\s+de|exceed(ed|ing)?|surpass(ed|ing)?|(?<!<|=)>)`;
    export const LessRegex = `(?:(less|plus\\s+(bas|petit|jeune)|moins|inf[ée]rieur)(\\s+([àa]|de|que))?|below|under|(?<!>|=)<)`;
    export const EqualRegex = `([ée]ga(l|ux)(\\s+[àa])?|(?<!<|>)=)`;
    export const MoreOrEqualPrefix = `((pas\\s+${LessRegex})|(au\\s+moins|[àa] partir de))`;
    ...
    export const MoreOrEqualSuffix = `((et|ou)\\s+(((more|greater|higher|plus(\\s+grand)?|sup[ée]rieur)((?!\\s+([àa]|que))|(\\s+([àa]|que)(?!((\\s+ou\\s+[èe]ga(l|ux)\\s+[àa])?\\s*\\d+)))))|((a plus de|au-dessus de)(?!\\s+than))))`;
    ...
    export const LessOrEqualSuffix = `((et|ou)\\s+(less|lower|plus petit|moins|inf[ée]rieur)((?!\\s+([àa]|de|que))|(\\s+([àa]|de|que)(?!(\\s*\\d+)))))`;
    export const NumberSplitMark = `(?![,.](?!\\d+))(?!\\s*\\b(et\\s+(${LessRegex}|${MoreRegex})|mais|ou|to)\\b)`;
    export const MoreRegexNoNumberSucceed = `((bigger|greater|more|plus(\\s+grand)?|sup[ée]rieur)((?!\\s+([àa]|que))|\\s+(([àa]|que)(?!(\\s*\\d+))))|((au-dessus|a plus)\\s+de)(?!(\\s*\\d+)))`;
    export const LessRegexNoNumberSucceed = `((less|lower|plus petit|moins|inf[ée]rieur)((?!\\s+([àa]|de|que))|\\s+(([àa]|de|que)(?!(\\s*\\d+))))|(below|under)(?!(\\s*\\d+)))`;
    export const EqualRegexNoNumberSucceed = `([èe]ga(l|ux)((?!\\s+([àa]))|(\\s+([àa]|que)(?!(\\s*\\d+)))))`;
