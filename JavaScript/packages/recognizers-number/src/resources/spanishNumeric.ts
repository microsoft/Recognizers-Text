...
export namespace SpanishNumeric {
    ...
    export const LessOrEqualSuffix = `((\\b(y|o)\\b\\s+(meno(s|r(es)?|inferior(es)?))((?!\\s+(alt[oa]|baj[oa]|que|del?|a))|(\\s+(que|del?|a)(?!(\\s*\\d+)))))|como\\s+m[áa]ximo)`;
    export const NumberSplitMark = `(?![,.](?!\\d+))`;
    export const MoreRegexNoNumberSucceed = `((m[áa]s|mayor(es)?|superior(es)?)((?!\\s+(que|del?|a))|\\s+((que|del?)(?!(\\s*\\d+))))|(por encima)(?!(\\s*\\d+)))`;
    ...
    export const TwoNumberRangeRegex1 = `entre\\s*((el|las?|los)\\s+)?(?<number1>(${NumberSplitMark}.)+)\\s*y\\s*((el|las?|los)\\s+)?(?<number2>(${NumberSplitMark}.)+)`;
