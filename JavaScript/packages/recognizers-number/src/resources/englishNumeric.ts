...
export namespace EnglishNumeric {
	...
	export const NegativeNumberTermsRegex = `(minus|negative)`;
	...
	export const AllIntRegex = `((${NegativeNumberTermsRegex}\\s+)?((((${TenToNineteenIntegerRegex}|(${TensNumberIntegerRegex}(\\s+(and\\s+)?|\\s*-\\s*)${ZeroToNineIntegerRegex})|${TensNumberIntegerRegex}|${ZeroToNineIntegerRegex}|${AnIntRegex})(\\s+${RoundNumberIntegerRegex})+)\\s+(and\\s+)?)*${SeparaIntRegex}))`;
