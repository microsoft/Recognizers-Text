...
export namespace EnglishNumeric {
	...
	export const NumberOrdinalRegex = `(first (one|1)|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth|fifteenth|sixteenth|seventeenth|eighteenth|nineteenth|twentieth|thirtieth|fortieth|fiftieth|sixtieth|seventieth|eightieth|ninetieth)`;
	export const RelativeOrdinalRegex = `((next|previous) (one|1)|(the second|next) to last|the one before the last( one)?|the last but one|(ante)?penultimate|last|next|previous)`;
	export const BasicOrdinalRegex = `(${NumberOrdinalRegex}|${RelativeOrdinalRegex})`;
	export const RelativeOrdinalFilterRegex = `(?<!-)(first|${RelativeOrdinalRegex})\\s*(${TwoToNineIntegerRegex}|[2-9]\\b)(?!\\s*${RoundNumberIntegerRegex})`;
	...
	export const MoreOrEqualSuffix = `((and|or)\\s+(more|greater|higher|larger|bigger)((?!\\s+than)|(\\s+than(?!(\\s*\\d+)))))`;
	export const LessOrEqualPrefix = `((no\\s+${MoreRegex})|(at\\s+most))`;
	...
	export const RelativeReferenceOffsetMap: ReadonlyMap<string, string> = new Map<string, string>([["last", ""],["next one", ""],["previous one", ""],["previous 1", ""],["the second to last", ""],["the one before the last one", ""],["next to last", ""],["penultimate", ""],["the last but one", ""],["antepenultimate", ""],["next", ""],["previous", ""]]);
	export const RelativeReferenceRelativeToMap: ReadonlyMap<string, string> = new Map<string, string>([["last", "end"],["next one", "current"],["previous one", "current"],["previous 1", "current"],["the second to last", "end"],["the one before the last one", "end"],["next to last", "end"],["penultimate", "end"],["the last but one", "end"],["antepenultimate", "end"],["next", "current"],["previous", "current"]]);
}
