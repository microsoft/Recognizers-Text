...
export namespace EnglishNumeric {
	...
	export const MoreRegex = `(bigger|greater|more|higher|larger|above|>)`;
	export const LessRegex = `(less|lower|smaller|below|<)`;
	export const EqualRegex = `(equal(\\s+to)?|=)`;
	...
	export const OneNumberRangeMoreRegex1 = `(${MoreOrEqual}|${MoreRegex})\\s*(than\\s+)?(the\\s+)?(?<number1>((?!((\\.(?!\\d+))|(,(?!\\d+)))).)+)`;
	export const OneNumberRangeMoreRegex2 = `(?<number1>((?!((\\.(?!\\d+))|(,(?!\\d+)))).)+)\\s*and\\s+(more|greater|higher|larger|bigger)`;
	export const OneNumberRangeLessRegex1 = `(${LessOrEqual}|${LessRegex})\\s*(than\\s+)?(the\\s+)?(?<number2>((?!((\\.(?!\\d+))|(,(?!\\d+)))).)+)`;
	export const OneNumberRangeLessRegex2 = `(?<number2>((?!((\\.(?!\\d+))|(,(?!\\d+)))).)+)\\s*and\\s+(less|lower|smaller)`;
