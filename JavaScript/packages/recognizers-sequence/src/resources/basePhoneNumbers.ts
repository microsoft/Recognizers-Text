...
export namespace BasePhoneNumbers {
	...
	export const USPhoneNumberRegex = `(((((\\B\\+)|\\b)1(\\s|-)?)|\\b)?(\\d{3}\\)[-\\s]?|\\(\\d{3}\\)[-\\s]?|\\b\\d{3}\\s?[-\\.]?\\s?)|\\b)\\d{3}\\s?[-\\.]?\\s?\\d{4}(\\s?(x|X|ext)\\s?\\d{3,5})?(?!(-\\s?\\d))\\b`;
	...
	export const SpecialUSPhoneNumberRegex = `(?<!(-|-\\s|\\d|\\)|\\)\\s|\\.))\\d{3}\\s?[-\\.]\\s?\\d{4}(?!(-\\s?\\d))\\b`;
