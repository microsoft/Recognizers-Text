...
export namespace BasePhoneNumbers {
	...
	export const USPhoneNumberRegex = `((((\\B\\+)|\\b)1(\\s|-)?)|\\b)?(\\(\\d{3}\\)\\s?|\\b\\d{3}\\s?[-\\.]?\\s?)\\d{3}\\s?[-\\.]?\\s?\\d{4}(x\\d{3,5})?(?!-)\\b`;
	export const CNPhoneNumberRegex = `((\\b00\\s?)?\\+?86\\s?-?\\s?)?(((\\b|(?<=86))\\d{2,5}\\s?-?\\s?|\\(\\d{2,5}\\)\\s?)\\d{4}\\s?-?\\s?\\d{4}(\\s?-?\\s?\\d{4})?|(\\b|(?<=86))\\d{3}\\s?-?\\s?\\d{4}\\s?-?\\s?\\d{4})(?!-)\\b`;
	...
	export const NLPhoneNumberRegex = `((((\\(\\s?(\\+\\s?|00)31\\s?\\)\\s?)|(((?<!\\d)\\+\\s?|\\b00)31\\s?))?(((\\b|(?<=31))0?\\d{1,3}\\s?-?|\\(\\s?0?\\d{1,3}\\s?\\)|\\(0\\)\\s?\\d{1,3}\\s?-?)\\s?[\\d\\s]{5,11}\\d))|\\b\\d{10,12})(?!-)\\b`;
	export const SpecialPhoneNumberRegex = `\\b(\\d{3,4}[/-]\\d{1,4}[/-]\\d{3,4})\\b`;
	export const TypicalDeductionRegexList = [ '^\\d{3}-\\d{2}-\\d{4}$','^\\d{5}-\\d{4}$' ];
