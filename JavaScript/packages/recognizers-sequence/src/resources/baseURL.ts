...
export namespace BaseURL {
	export const SchemeRegex = `((https?|ftp):\\/\\/)`;
	export const PortRegex = `(:\\d{1,5})`;
	export const UrlPrefixRestrictRegex = `(?<=\\s|[\\'\"\"\\(\\[:.]|^)`;
	export const UrlPrefixRegex = `${UrlPrefixRestrictRegex}${SchemeRegex}?[-a-zA-Z0-9:%._\\+~#=]{1,256}\\.`;
	export const UrlSuffixRegex = `${PortRegex}?([/#][-a-zA-Z0-9:%_\\+.~#?!&//=]*)?(?!:)`;
	export const UrlRegex = `${UrlPrefixRegex}[a-zA-Z]{2,18}${UrlSuffixRegex}`;
	export const IpUrlRegex = `${UrlPrefixRestrictRegex}${SchemeRegex}(${BaseIp.Ipv4Regex}|localhost)${UrlSuffixRegex}`;
