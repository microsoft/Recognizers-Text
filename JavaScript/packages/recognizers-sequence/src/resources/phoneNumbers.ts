...
export namespace PhoneNumbers {
	...
	export const GeneralPhoneNumberRegex = `(\\b((\\d[\\s-/]?){8,})\\b)`;
	export const BrazilPhoneNumberRegex = `(((\\B\\(\\s?)|\\b)\\d{2,3}(\\s?\\))?\\s?)\\s?\\d{4,5}-?\\d{3,5}\\b`;
	export const UkPhoneNumberRegex = `(((\\b(00)|\\B\\+)\\s?)?\\d{2}\\s?(\\(0\\))?\\d{3,5}[/-]?\\s?(\\d{7,8}|\\d{3}\\s?\\d{3})\\b)`;
