        ...
        describe('Timex helpers', () => {
            describe('Timex expand to range of start and end Timex', () => {
                it('expandDateTimeRange (short range)', () => {
                    const timex = new Timex('(2017-09-27,2017-09-29,P2D)');
                    const range = timexHelpers.expandDateTimeRange(timex);
                    (new Timex(range.start)).timex.should.equal('2017-09-27');
                    (new Timex(range.end)).timex.should.equal('2017-09-29');
                });
                it('expandDateTimeRange (long range)', () => {
                    const timex = new Timex('(2006-01-01,2008-06-01,P882D)');
                    const range = timexHelpers.expandDateTimeRange(timex);
                    (new Timex(range.start)).timex.should.equal('2006-01-01');
                    (new Timex(range.end)).timex.should.equal('2008-06-01');
                });
                it('expandDateTimeRange (including time)', () => {
                    const timex = new Timex('(2017-10-10T16:02:04,2017-10-10T16:07:04,PT5M)');
                    const range = timexHelpers.expandDateTimeRange(timex);
                    (new Timex(range.start)).timex.should.equal('2017-10-10T16:02:04');
                    (new Timex(range.end)).timex.should.equal('2017-10-10T16:07:04');
                });
                it('expandDateTimeRange (month)', () => {
                    const timex = new Timex('2017-05');
                    const range = timexHelpers.expandDateTimeRange(timex);
                    (new Timex(range.start)).timex.should.equal('2017-05-01');
                    (new Timex(range.end)).timex.should.equal('2017-06-01');
                });
                it('expandDateTimeRange (year)', () => {
                    const timex = new Timex('1999');
                    const range = timexHelpers.expandDateTimeRange(timex);
                    (new Timex(range.start)).timex.should.equal('1999-01-01');
                    (new Timex(range.end)).timex.should.equal('2000-01-01');
                });
                it('expandTimeRange', () => {
                    const timex = new Timex('(T14,T16,PT2H)');
                    const range = timexHelpers.expandTimeRange(timex);
                    (new Timex(range.start)).timex.should.equal('T14');
                    (new Timex(range.end)).timex.should.equal('T16');
                });
            });
            ...
            describe('Timex convert to Date or Time objects', () => {
                ...
                it('timeFromTimex', () => {
                    const timex = new Timex('T00:05:00');
