...
export namespace BaseNumbers {
    ...
    export const IntegerRegexDefinition = (placeholder: string, thousandsmark: string) => {
 return `(((?<!\\d+\\s*)-\\s*)|((?<=\\b)(?<!(\\d+\\.|\\d+,))))\\d{1,3}(${thousandsmark}\\d{3})+(?=${placeholder})`; 
};
    export const DoubleRegexDefinition = (placeholder: string, thousandsmark: string, decimalmark: string) => {
