...
export namespace EnglishNumeric {
	...
	export const DoubleWithMultiplierRegex = '(((?!\\d+\\s*)-\\s*)|((\\s|^)(?=\\b)))\\d+\\.\\d+\\s*(K|k|M|G|T|B|b)(?=\\b)';
	export const DoubleExponentialNotationRegex = '(((?!\\d+\\s*)-\\s*)|((?=\\b)(((\\d+\\.\\d)|\\d+)|(?!\\d+\\.))))(\\d+(\\.\\d+)?)e([+-]*[1-9]\\d*)(?=\\b)';
	export const DoubleCaretExponentialNotationRegex = '(((?!\\d+\\s*)-\\s*)|((\\s|^)(?=\\b)))((\\d+\\.\\d)|\\d+)+(?!(\\.\\d+))(?=(?=\\b))\\^([+-]*[1-9]\\d*)(?=\\b)';
	export const DoubleDecimalPointRegex = (placeholder: string) => { return `(((?!\\d+\\s*)-\\s*)|((?=\\b)))\\d+\\.\\d+(?!\\.)(?=${placeholder})`; }
	...
	export const DigitalNumberRegex = '((?=\\b)(hundred|thousand|million|billion|trillion|dozen(s)?)(?=\\b))|(?:(\\d{1}|\\b))(k|t|m|g|b)(?=\\b)';
